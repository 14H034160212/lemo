...
Across the four primary stress tests (Table~\ref{tab:comparison_results}), Fusion-Conflict \emph{saturates} the held-out split for both backbones---meaning the policy matches the symbolic oracle's ground-truth on a benchmark that GPT-4o resolves at only $56.0\%$ on V3 and that the strongest non-Fusion baseline (Fusion-LRA) plateaus at $0.705$. The headline result is the \emph{gap} between the structural prior and these alternatives, not the absolute number. The \textbf{Variant~4 multi-law stacking} split remains the hardest condition: composing $2$--$5$ equivalence rewrites still degrades the $1.5$B backbone to $0.773$ while the $8$B backbone closes that gap, indicating that contradictory premises are now reliably handled but deep compositional rewrites still benefit from capacity. The Phase~2 RLVF-Lean extension (Section~\ref{sec:phase2}) targets this regime with a Lean kernel verifier beyond \textsc{ForwardChain}'s propositional reach.
...
\paragraph{Pilot study and engineering note.}
A hand-crafted pilot on the Socrates example (8 policy traces; full breakdown in Appendix~\ref{app:lean-pilot}, source in Appendix~\ref{app:lean-demo}) achieves $8/8$ kernel agreement at $0.36$\,s/trace on CPU. One non-trivial finding: Lean's \texttt{sorry} closes a goal with exit code~$0$, so we patch the bridge to treat any \texttt{sorry}/\texttt{admit} as reward~$-1$ to preserve soundness; end-to-end RL would amortise dispatch via a \textsc{LeanDojo}-style persistent server~\citep{yang2023leandojo}.
...
RLVF-Lean operationalises the neuro-symbolic stance~\citep{marcus2019rebooting,davis2024benchmarks,davis2024mathematics,davis2024gpt4o1} inside an RL training loop: the Lean kernel supplies step-level verification (Eq.~\ref{eq:lean-reward}) so verification is learned as part of the policy, not elicited at inference. End-to-end empirical evaluation on math/proof benchmarks is left to future work.
