...
\paragraph{Why DPO matters.} Fusion-LRA (SFT only) plateaus at $0.705$ on Variant~3 because SFT teaches the \emph{form} of the contradiction check but still occasionally hallucinates success---a ``shallow alignment'' failure. Adding DPO sharpens the halt-on-contradiction decision boundary and pushes Variant~3 to $1.000$, supporting the role of preference optimisation as a \emph{robustness hardener} rather than a marginal improvement.
...
We presented a controlled framework for analysing and enhancing the structural robustness of rule-based LLM reasoning. Across BERT, Qwen2, TinyLlama, and Qwen3-8B, we showed that contemporary models satisfy semantic-form invariance yet collapse under essential-rule deletion and explicit contradictions---the \textbf{Logic Inertia} failure mode. Our \textbf{Conflict-Aware Fusion} framework, trained by a four-stage SFT $\to$ DPO $\to$ LIRE $\to$ RLVF pipeline, closes this gap and reaches accuracy~$1.000$ on base, Variant~2, and Variant~3 for both $1.5$B and $8$B backbones. The Phase~2 RLVF-Lean extension demonstrates a sound upgrade path to a formal-proof-assistant verifier with $99.0\%$ agreement on multi-step T-derivations. The benchmark, code, and Lean verifier are released to support reproducibility.
