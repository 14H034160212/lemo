...
premise is applied at $0.999$ and its contrapositive at $0.199$; GPT-4.1
answers it correctly $0$ times in $600$ attempts and GPT-5.2 $0$ in $450$, and
a wrong step takes the step after it down in $76$--$100\%$ of cases. The same
models complete the rest of the chain at ceiling, so a total failure on one
inference form surfaces as an aggregate between $0.80$ and $0.93$---a
degradation where the measurement shows an absence. Raising the reasoning
budget never recovers it. Reaching it required
...
contrapositive at $0.199$, with the upstream steps at ceiling; GPT-4.1 answers
the contrapositive correctly $0$ times in $600$ attempts and GPT-5.2 $0$ in
$450$, and the downstream query fails alongside it in $76$--$100\%$ of cases
(\S\ref{sec:chain}).
...
in three of eight configurations, reasoning effort never recovers it, and the
...
GPT-5-nano  & medium  & 1 & 0.998 & 1.000 & 0.993 & 0.080 & 0.080 & 0.844 \\
GPT-5-mini  & default & 3 & 1.000 & 1.000 & 1.000 & 0.602\,$\pm$\,0.056 & 0.602 & 0.934 \\
GPT-5-mini  & medium  & 1 & 1.000 & 1.000 & 1.000 & 0.507 & 0.507 & 0.918 \\
...
MT $0.199$. GPT-4.1 answers MT correctly $0$ times in $600$ attempts and
GPT-5.2 at medium effort $0$ in $450$, with their upstream steps at $1.000$ and
...
\texttt{reasoning\_effort} to medium never improves MT: $-0.016$ on GPT-5-nano
(inside its $\pm0.043$ run-to-run spread), $-0.095$ on GPT-5-mini and $0.000$ on
GPT-5.2, at $42$--$304$ reasoning tokens per question. The one model that is not
near-zero, GPT-5-mini at $0.602$, is the one that loses the most. Whatever the
...
$0.199$---GPT-4.1 $0$ times in $600$ attempts---with the downstream query
failing alongside it, all of it invisible inside aggregates of
$0.80$--$0.93$. Where a perturbation fixes the label, accuracy alone cannot
