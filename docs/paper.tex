...
To address it, we propose \textbf{Conflict-Aware Fusion (Fusion-Conflict)}, grounded in the \textbf{Cognitive Structure Hypothesis}: robust reasoning requires an explicit structural separation between premise validation and deductive execution. The framework operationalises this via a dual-process architecture trained by a four-stage pipeline (SFT $\to$ DPO $\to$ LIRE $\to$ RLVF). Our contributions are: \textbf{(i)} a diagnostic benchmark that disentangles redundancy, incompleteness, and contradiction, and the formalisation of \emph{Logic Inertia} as a structural failure mode reproduced across BERT, Qwen2/3, TinyLlama, GPT-4o, and Gemma-3-4B; \textbf{(ii)} \textbf{LIRE} (\emph{Logical Invariance REgularisation}), a symmetric-KL regulariser that enforces output-distribution invariance across logically equivalent rule formulations; \textbf{(iii)} \textbf{RLVF} (\emph{Reinforcement Learning from Verification Feedback}), a new RL algorithm that replaces RLHF's noisy human or learned reward with a deterministic symbolic-oracle reward, jointly optimising structural invariance and sensitivity in a single REINFORCE update; \textbf{(iv)} a sound \textsc{Lean~4} extension (\textbf{RLVF-Lean}) that elevates the oracle from a propositional forward-chainer to a formal proof-assistant kernel, with $99.0\%$ verifier agreement on $187$ multi-step T-derivations from our benchmark; and \textbf{(v)} the \textbf{Fusion-Conflict} pipeline that integrates the four stages and \emph{saturates} the held-out base, Variant~2 (essential-rule deletion), and Variant~3 (contradiction) splits for both $1.5$B and $8$B backbones---closing a gap that leaves untreated baselines at $0.250$ on V2 and $0.000$ on V3, and that GPT-4o and Gemma-3-4B-IT resolve in only $56.0\%$ and $43.9\%$ of V3 contradiction cases respectively. The synthetic structure is intentional: the benchmark is calibrated to \emph{expose} Logic Inertia, so the headline contrast lies in the \emph{collapse} pattern of strong baselines rather than in the absolute ceiling.
...
\textsc{RLVF} is the new RL algorithm we introduce in this work. The key design choice is to replace RLHF's noisy human preferences or learned reward models with a \emph{deterministic symbolic oracle}, eliminating reward-model bias and enabling provably unbiased policy gradients on the rule-based reasoning regime. We formulate inference as a contextual bandit: at each step, sample $a_s\sim\pi_\theta(\cdot|\mathbf{x})$, query the oracle $a^*=\textsc{ForwardChain}(\mathbf{x})$, and assign $r=+1$ if $a_s=a^*$ else $-1$. The policy is optimised by REINFORCE~\citep{williams1992simple} with an EMA baseline $b$:
\begin{equation}
...
We presented a controlled framework for analysing and enhancing the structural robustness of rule-based LLM reasoning. Across BERT, Qwen2, TinyLlama, Qwen3-8B, GPT-4o, and Gemma-3-4B-IT we showed that contemporary models satisfy semantic-form invariance yet collapse under essential-rule deletion and explicit contradictions---the \textbf{Logic Inertia} failure mode. We introduced two named methodological contributions: \textbf{LIRE}, a symmetric-KL regulariser that enforces invariance across logically equivalent rule formulations, and \textbf{\textsc{RLVF}} (\emph{Reinforcement Learning from Verification Feedback}), a new RL algorithm that replaces RLHF's learned reward model with a deterministic symbolic oracle, jointly optimising structural invariance and sensitivity in a single REINFORCE update. Combined in the \textbf{Conflict-Aware Fusion} pipeline (SFT $\to$ DPO $\to$ LIRE $\to$ \textsc{RLVF}), they \emph{saturate} the held-out base, Variant~2, and Variant~3 splits for both $1.5$B and $8$B backbones on a benchmark calibrated to expose Logic Inertia (where untreated models fall to $0.000$ on V3, GPT-4o resolves only $56.0\%$ of V3 contradiction cases, and Gemma-3-4B-IT only $43.9\%$). The Phase~2 \textbf{\textsc{RLVF-Lean}} extension generalises the oracle reward from a propositional forward-chainer to a Lean~4 kernel, achieving $99.0\%$ agreement on $187$ multi-step T-derivations. The benchmark, training pipeline, and Lean verifier are released to support reproducibility.
