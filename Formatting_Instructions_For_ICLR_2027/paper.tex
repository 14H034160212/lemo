...
\begin{table*}[t]
    \centering
    \caption{Trained methods on Qwen2-1.5B, re-measured in September 2026 on the
    current benchmark with the released checkpoints. \textbf{V3 and V3-mixed
    measure the same capability and disagree completely.} Variant~3 is
    single-class---under the conservative semantics of \S\ref{sec:method} every
    query on a contradictory instance is False---so its majority-class baseline
    is $1.000$ and a model that answers False on sight scores perfectly there
    while learning nothing. V3-mixed (\S\ref{sec:v3control}) interleaves
    contradictory instances with consistent ones of the same surface form;
    its baseline is $0.5835$. Only Fusion-LRA exceeds that baseline. Every row
    reports the evaluation path used, because a two-logit classification head
    and free generation are not equally hard.}
...
The comparison between objectives over a shared Stage-1 base was this work's
original argument, and \S\ref{sec:v3control} removes most of it: on the
two-class control only conflict-aware SFT exceeds a constant responder. The
stage-wise figures from the earlier draft are kept in
Appendix~\ref{app:stagewise}, measured before the 2026-03-31 benchmark
regeneration and not re-verified, so they document what was tried rather than
support any claim. What survives the control is reported in
Table~\ref{tab:comparison_results} and \S\ref{sec:v3control}.
...
did not survive them. Of four trained objectives only conflict-aware SFT exceeds
a constant responder on a two-class control, and it does so at
$0.698 \pm 0.054$ over six seeds against a $0.5835$ baseline---weaker at 8B, not
stronger, and resting partly on a surface cue that co-occurs with contradiction
in training with probability $1.000$. A symbolic solver scores $1.000$ on every
split of this benchmark, so nothing here argues for a learned approach where the
premises are already formalised.

What the benchmark does well is diagnose. Matched arms that differ only in
inference form show GPT-5.2 at medium reasoning effort applying a negated
premise perfectly and its contrapositive not once in 150 attempts, with the
downstream query failing alongside it in $96$--$100\%$ of cases---a total
failure, not the degradation the literature reports, and invisible inside an
aggregate of $0.797$. Isolating it required pairing instances, sampling chains
rather than reusing thirty structures, and reporting seeds rather than
questions. We think that is the transferable part: on perturbation-style
benchmarks, where the perturbation fixes the label, an accuracy alone cannot
