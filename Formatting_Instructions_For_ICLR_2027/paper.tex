...
reported separately. GPT-5.2 at medium reasoning effort applies a rule with a
negated premise perfectly and its contrapositive not once in $150$ attempts,
with the steps before it at $0.940$ and the step after it failing alongside it
in $96$--$100\%$ of cases---a total failure where the literature reports a
degradation, and one an aggregate of $0.797$ conceals. Reaching it required
...
instance in \textsc{LogicBench}, atomic skills and their quantified variants in
\textsc{LogicAsker}, a single two-premise inference in
\textsc{RULEBREAKERS}---and none of these measure error propagation or per-step
correctness. We pair instances that differ only in the form of one inference,
fix each query's position in the chain, and report the primary failure and the
propagated one separately. GPT-5.2 at medium reasoning effort applies a rule
with a negated premise perfectly ($1.000$) and its contrapositive not once in
$150$ attempts ($0.000$), with upstream steps at $0.940$ and the downstream
query failing alongside it in $96$--$100\%$ of cases. The literature reports
modus tollens as harder than modus ponens; inside a chain, at maximum effort,
it is absent, and the aggregate reads $0.797$.
