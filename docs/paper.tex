...
This work makes three primary contributions. First, it establishes a \textbf{diagnostic benchmark} that enables precise measurement of structural robustness in rule-based reasoning, disentangling the effects of redundancy, incompleteness, and contradiction. Second, it identifies and formalizes the phenomenon of \textbf{Logic Inertia}, revealing a fundamental limitation in current LLM reasoning that remains invisible under conventional benchmarks. Third, it introduces the \textbf{Conflict-Aware Fusion (Fusion-Conflict)} framework, demonstrating that incorporating explicit premise-validation mechanisms can transform model behavior, achieving accuracy~$1.000$ on the base task, on essential-rule deletion (Variant~2), and on contradiction handling (Variant~3) for both the $1.5$B and $8$B backbones, where untreated baselines achieve only $0.250$ and $0.000$ on Variants~2 and~3 respectively.
...
Across the four primary stress tests (base, Variant~1 redundant deletion, Variant~2 essential deletion, Variant~3 contradiction), Fusion-Conflict reaches accuracy~$1.000$ on the held-out test split for both the $1.5$B and $8$B backbones (Table~\ref{tab:comparison_results}), confirming that the four-stage pipeline closes the gaps left by SFT, DPO, and LIRE individually. The \textbf{Variant~4 multi-law stacking} split remains the most challenging condition: composing $2$--$5$ equivalence rewrites simultaneously still degrades the smaller $1.5$B backbone to $0.773$ on this split, while the $8$B backbone reaches $1.000$. This indicates that while contradictory premises are now reliably handled, deep compositional rewrites still benefit from larger model capacity. The Phase~2 RLVF-Lean extension (Section~\ref{sec:phase2}) targets this regime by replacing the propositional oracle with a formal proof assistant capable of verifying multi-step proofs at a granularity beyond what \textsc{ForwardChain} provides.
