...
This section concerns a second failure, which the benchmark was originally
built for: given premises that contradict each other, a model continues to
deduce from them instead of halting---it notices the inconsistency in its own
trace and answers anyway. We call that \textbf{Logic Inertia}, and it is not
rule loss: the rule is applied, the premise set is what has failed. The
baseline pattern is uniform across BERT, Qwen2 and TinyLlama (Table~\ref{tab:all_models_deltas}, Appendix~\ref{app:baseline-table}): all three stay at $1.000$ on the base task and Variant~1, drop to $0.25$--$0.295$ under essential-rule removal (Variant~2), and collapse to $0.000$ under explicit contradictions (Variant~3).
