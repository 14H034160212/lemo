...
%%% CAMERA-READY AUTHOR BLOCK: kept out of the repository for double-blind
%%% review; restore from AUTHORS.local.txt together with \iclrfinalcopy.
%\author{%
...
\begin{abstract}
Large language models (LLMs) achieve high accuracy on many reasoning benchmarks but remain brittle under structural perturbations of rule-based systems. We introduce a controlled diagnostic framework with four orthogonal stress tests---redundant vs.\ essential rule deletion, contradictory-rule injection, logic-preserving rewrites, and multi-law stacking---and use it to expose \emph{Logic Inertia}: the systematic tendency of LLMs to persist along learned deductive trajectories even when foundational premises are inconsistent. The collapse is sharp: untreated baselines fall from accuracy~$1.00$ on the base task to $0.00$ on contradiction injection (instance-level exact match; \S\ref{sec:method}). It also survives a full model generation---current frontier models reach only $82.5\%$ (GPT-5.2) and $67.9\%$ (Claude~Opus~5) on contradictions---and resists the obvious inference-time fixes: a verification-first few-shot prompt makes both models \emph{worse} on exactly that axis (McNemar $p{=}0.022$ and $p{<}10^{-4}$ on $800$ paired questions), as does an external symbolic contradiction guard. We propose \textbf{Conflict-Aware Fusion}, which trains verification-before-deduction in as a structural prior: a shared SFT base establishes the verification preamble, and three objectives are trained from it and compared---DPO sharpens the halt-on-contradiction boundary; Logical Invariance REgularisation (LIRE) enforces symmetric-KL invariance across equivalent rule formulations but overshoots into insensitivity; and Reinforcement Learning from Verification Feedback (RLVF), our final model, uses a symbolic forward-chaining engine as a deterministic oracle reward that optimises both jointly in one run. The pipeline saturates all four primary stress tests for both $1.5$B and $8$B backbones. A Phase~2 extension replaces the propositional oracle with a \textsc{Lean~4} kernel, attaining $99.0\%$ kernel agreement on the $105$ classically-derivable questions of a stratified $187$-question Lean-translated sample (the T/F asymmetry is analysed in \S\ref{sec:phase2}) and giving a sound upgrade path to formally verified RL training. We scope the claim honestly, and the scoping evidence is negative: zero-shot on LogicNLI, MNLI, FOLIO and ZebraLogic, every $1.5$B checkpoint collapses to a constant predictor at the majority-class baseline, the $8$B checkpoint collapses the opposite way on LogicNLI, and only FOLIO at $8$B ($61.9\%$ vs.\ $50\%$ chance) shows real transfer. What the pipeline buys is \emph{structural} logical robustness inside the propositional regime, not a general reasoning upgrade. The benchmark, training pipeline, and Lean verifier are publicly released.\footnote{Anonymous code/data repository: \url{https://anonymous.4open.science/r/lemo-D708/README.md}}
\end{abstract}
...
consequences. On matched arms differing only in inference form, GPT-5.2 at
medium reasoning effort applies a rule with a negated premise perfectly
($1.000$) and its contrapositive not once in 150 attempts ($0.000$), with the
upstream steps at $0.940$---and the downstream query fails with it in $96$--%
$100\%$ of cases.
...
\paragraph{Contributions.} \textbf{(i)}~A controlled benchmark and the
characterisation of Logic Inertia, with evidence that it persists in current
frontier models and resists inference-time mitigation. \textbf{(ii)}~Matched
inference-form arms that separate a primary reasoning failure from its
propagation, showing a total rather than degraded contraposition failure hidden
inside a plausible aggregate. \textbf{(iii)}~An evaluation protocol---two-class
controls, majority baselines, prediction distributions, answer-parse rates,
per-run provenance---without which the headline numbers of an earlier draft of
this work could not be distinguished from a constant predictor, and under which
most of them did not survive.

\paragraph{Scope.} The capability is propositional premise validation. Against
an untrained backbone it transfers to premise-consistency tasks
(LogicNLI $+0.210$, $p{=}1.2{\times}10^{-9}$; MNLI-contradiction $+0.152$,
$p{=}1.2{\times}10^{-7}$) and not at all to first-order reasoning
(FOLIO $-0.019$, $p{=}0.40$). It does not improve with scale, and
\S\ref{sec:shortcut} shows part of what we measure is a surface cue present in
training with probability $1.000$.
...
These designs deliberately isolate each rule, which is what makes them
diagnostic. It also means they do not measure what a weak rule costs once it is
\emph{embedded}. In a chain, a single failed step does not contribute one
error---it caps every conclusion downstream of it, and an aggregate score mixes
those propagated failures with primary ones. \S\ref{sec:v3control} reports a
frontier model answering the first two steps of a four-step chain correctly and
the last two incorrectly, where only the third is a reasoning failure and the
fourth is its consequence. That compound cost is the quantity we measure.
...
\textsc{RULEBREAKERS} also supplies the methodological precedent for our
controls. Because its perturbation determines the label---every rulebreaker is
False and every matched non-rulebreaker True---per-condition accuracy is
saturated by a constant response, so it scores paired instances jointly and a
constant policy earns zero. We arrived at the same requirement from the
opposite direction, by finding that our own single-class splits had awarded
$1.000$ to a model answering \texttt{False} on 4000/4000 questions
(\S\ref{sec:v3control}).
...
(Eq.~\ref{eq:rlvf}); \S\ref{sec:v3control} reports that it collapses to a
...
\paragraph{The capability gets worse with scale.} We trained the same
conflict-aware objective on Qwen3-8B, four seeds against six at $1.5$B. The
larger model is clearly better on the splits that do not require contradiction
detection---base $0.9075$ against $0.870$, V2 $0.8023$ against $0.610$---and
significantly worse on the control:

\begin{center}\small
\begin{tabular}{lccc}
\toprule
& \textbf{accuracy} & \textbf{discrimination gap} & \textbf{seeds} \\
\midrule
Qwen2-1.5B & $0.6979 \pm 0.0542$ & $0.420 \pm 0.200$ & 6 \\
Qwen3-8B & $0.6323 \pm 0.0282$ & $0.146 \pm 0.071$ & 4 \\
\bottomrule
\end{tabular}
\end{center}

Welch $p{=}0.0189$ on accuracy and $p{=}0.0096$ on the gap; Mann--Whitney
$p{=}0.0333$ and $p{=}0.0095$, reported because the variances differ. The 8B
model remains above the $0.5835$ baseline ($p{=}0.0204$), so this is a weaker
capability rather than none: the larger model learns more of the task and less
of this part of it. Its seed variance is also a third of the smaller model's
($0.071$ against $0.200$), so $1.5$B is erratically better while 8B is
consistently mediocre.

\textbf{A single seed would have reported this wrongly in both directions.} The
first 8B run we trained happened to be the worst of the four ($0.081$ against a
mean of $0.146$) and supported a stronger claim than the data does; the best
($0.247$) is within one standard deviation of the $1.5$B mean and would have
supported none.
...
chain plus exactly one extra negated fact.

\begin{itemize}[leftmargin=14pt,itemsep=1pt,topsep=2pt]
\item \textbf{contra}: the negated fact contradicts the chain $\Rightarrow$ all False.
\item \textbf{consistent-disj}: it negates one disjunct; the other still fires $\Rightarrow$ nothing breaks.
\item \textbf{consistent-mixed}: a chain with one rule removed, plus a negated fact about something that chain can no longer derive.
\end{itemize}

The third class is the discriminating one: the extra fact is frequently the
...
\begin{table}[h]
\centering\small
\caption{Per-class behaviour on the control; Fusion-LRA is mean\,$\pm$\,s.d.\ over
six training seeds. The discriminating quantity is the gap between the
consistent and contradictory True-rates: it is positive for every seed and
averages $0.420 \pm 0.200$ ($p{=}0.0018$ against zero, $n{=}6$). Its size varies
a great deal, and the variance sits almost entirely in
\textbf{cons.-disj}---plainly consistent instances, which different seeds accept
between $17\%$ and $93\%$ of the time. Contradiction detection itself is the
stable part ($0.903 \pm 0.108$). \textsc{RLVF} is mean\,$\pm$\,s.d.\ over four seeds and answers False on
essentially everything: its discrimination gap is $0.014 \pm 0.018$, its
accuracy $0.5830 \pm 0.0065$ is statistically indistinguishable from the
$0.5835$ baseline ($p{=}0.44$), and two of the four seeds answer False on all
$12{,}000$ questions, landing exactly on the baseline with a gap of $0.000$. The
collapse is the objective's fixed point, not a bad draw---its seed variance is
an order of magnitude smaller than Fusion-LRA's. Its $1.000$ on Variant~3 is the
constant-False artefact the control was built to expose, and correcting the
reward oracle changed which constant it collapsed to, not that it collapsed.}
\label{tab:v3control}
\begin{tabular}{lccc}
\toprule
...
\subsection{Objective ablation: what each branch buys and gives up}
\label{sec:ablation-stagewise}
Table~\ref{tab:stage_wise} compares the three objectives on the four most diagnostic splits (base, V2 essential deletion, V3 contradiction, V4-multi rewrites) for both backbones, isolating what each contributes over the shared Stage-1 base.

% Table~\ref{tab:stage_wise} moved to Appendix~\ref{app:stagewise}: its figures
% predate the 2026-03-31 benchmark regeneration and have not been re-verified.


Three observations follow. \textbf{(i)} LIRE alone is harmful on V2/V3: its symmetric-KL term suppresses \emph{any} distribution shift across rule reformulations, including those genuine logical change requires---a clean instance of the invariance--sensitivity trade-off (Eq.~\ref{eq:lire}). \textbf{(ii)} \textsc{RLVF} dissolves the trade-off rather than repairing LIRE: one oracle reward (Eq.~\ref{eq:rlvf}) reinforces V4 invariance and V2/V3 sensitivity with the same gradient, so the fix is a better objective, not an extra stage. \textbf{(iii)} SFT-only at $8$B still falls to $0.656$ on V3---Logic Inertia is structural, not capacity-bound, whether or not scale-driven jumps are genuine emergence~\citep{wei2022emergent} or measurement artefacts~\citep{schaeffer2023mirage}. Finally, the two ``Stage~1'' numbers are \emph{not} contradictory: Fusion-LRA in Table~\ref{tab:comparison_results} (Base $0.988$) is SFT \emph{with} the conflict-aware preamble, whereas the $0.525$ entry in Appendix~\ref{app:ablation} is the same backbone on a vanilla \emph{Mixed-Aug} corpus \emph{without} it. The preamble, not the data mix, preserves base accuracy.

\paragraph{Rigid protocol or genuine robustness?} An earlier draft answered
this from the ablation alone. That answer does not hold: the protocol string
is emitted almost everywhere, including on instances with nothing to detect,
so its presence cannot separate the two hypotheses. The two-class control of
\S\ref{sec:v3control} is the test that can, and Table~\ref{tab:v3control}
reports what it finds---genuine discrimination for conflict-aware SFT, none for
any other objective, and a near-chance boundary on the hardest class.
\S\ref{sec:frontier_prompt} supplies the external control: handing a frontier
model that exact protocol as a prompt \emph{degrades} contradiction handling.

\section{Discussion}

\paragraph{Structure, not data exposure.} SFT alone (Fusion-LRA) plateaus at $0.705$ on V3: it teaches the \emph{form} of the contradiction check but still hallucinates success. Fusion-Conflict dominates both the data-balanced baseline (Mixed-Aug, $0.972$ V3 but $0.525$ base) and the over-conservative one (RA-CoT, $0.690$/$0.263$): reliability follows from \emph{structural organisation}---the verification preamble acting as a circuit breaker---not from data exposure alone.

\paragraph{An unbiased reward is not a low-variance one.} We initially argued
that because the oracle reward is exactly computable, deterministic, and free of
a reward-hacking surface, the clipping in PPO~\citep{schulman2017proximal} and
the group-relative normalisation in GRPO~\citep{shao2024deepseekmath} are
unnecessary, and used plain REINFORCE with an EMA baseline (Eq.~\ref{eq:rlvf}).
That argument is wrong and we withdraw it. With no trust region the EMA baseline
climbed to $+0.695$, so a wrong trace was penalised $5.6\times$ more than a
correct one was rewarded, and the policy degenerated into non-parsing output at
step~$250$. Adding a KL penalty against the initialisation stabilised training
for the full $600$ steps. Unbiasedness constrains the reward's expectation; it
says nothing about the variance of the gradient estimator, which is what the
trust region exists to control. A second failure mode is not a variance issue at
all: on a contradictory instance, ``answered False after detecting the
contradiction'' and ``answered False unconditionally'' receive \emph{identical}
reward, so an outcome-only signal is degenerate for the capability we are trying
to install, and \textsc{RLVF} converges to the constant
(Table~\ref{tab:v3control}). Demonstration supplies what the reward cannot.

\paragraph{Train- vs.\ inference-time mitigation.} Inference-time guards~\citep{vericot2026,xiong2025deliberate,premiseRAG2025} re-invoke a verifier on every deployment query; our prior is amortised into the weights at training time, so V2/V3 robustness costs nothing at runtime, and the Phase~2 Lean kernel is needed only during training.
...
\label{sec:external}
Saturating a synthetic benchmark invites the objection that the policy has memorised a template family. We evaluate the trained checkpoints zero-shot, with no in-domain examples, on four external datasets: \textbf{LogicNLI}~\citep{tian-etal-2021-diagnosing} and the contradiction subset of \textbf{MNLI}~\citep{N18-1101}, close to the premise-consistency behaviour we train; and \textbf{FOLIO}~\citep{han2024folio} (first-order-logic entailment; train$+$validation minus \texttt{Uncertain} rows, $811$ items) and \textbf{ZebraLogic}~\citep{lin2025zebralogic} (constraint-satisfaction puzzles; the public test labels are withheld, so we solve $313$ puzzles with our own solver and build two balanced T/F probes each), whose task formats we never trained on.
...
Three things follow, and they bound our claim rather than support it. \textbf{(i)}~At $1.5$B the prior does not transfer at all: all three checkpoints degenerate into constant-\texttt{False} predictors, scoring exactly the majority-class baseline on LogicNLI and chance on MNLI. \textbf{(ii)}~At $8$B the collapse inverts rather than resolves ($96\%$ of LogicNLI items labelled True, accuracy well below baseline), even though that checkpoint saturates every split of our own benchmark. \textbf{(iii)}~The one genuine positive is FOLIO at $8$B ($0.619$ vs.\ $0.500$ chance); ZebraLogic---a reasoning form with no premise-consistency component---moves at neither scale.

What the pipeline demonstrably installs is robustness of the \emph{verify-then-deduce} structure \emph{within} the propositional regime it is trained on, reliably across backbones; it is not a general reasoning upgrade, and the synthetic saturation of \S\ref{sec:ablation-stagewise} is not evidence that contradiction handling is solved in natural language. This also sharpens \S\ref{sec:frontier_prompt}: the trained prior beats every inference-time alternative \emph{on the structural axis it targets}, and that axis is narrower than ``reasoning''.

\subsection{Formal verification of the benchmark's labels}
\label{sec:phase2}
We translate instances to Lean~4 and check each query against the kernel, as an
audit of our own oracle: every other label in this benchmark comes from the
forward-chainer that also computes the RLVF reward, which couples training and
evaluation. Agreement is $0.9965$ on derivable queries in the five-domain
corpus ($n{=}1660$) and $0.6119$ on the corpus the rest of this paper uses, so
the headline figure is a property of that dataset rather than of the
translation; on non-derivable queries agreement is $0.2213$, because proving
that something does \emph{not} follow requires the closed-world assumption and
our translator does not encode it. \textbf{We therefore do not use Lean as an
independent adjudicator}---at these rates it would add noise. An RL variant
rewarded by the kernel is described in the earlier draft but was never
implemented, and we withdraw it (Appendix~\ref{app:leandetail}).

\section{Conclusion}

We diagnosed \textbf{Logic Inertia}---models persisting along learned deductive trajectories under essential-rule deletion and contradictions---and closed it with \textbf{LIRE} (trace-level symmetric-KL invariance) and \textbf{\textsc{RLVF}} (a deterministic symbolic-oracle reward replacing RLHF's learned reward model), combined in the \textbf{Conflict-Aware Fusion} pipeline, which saturates the held-out base/V2/V3 splits at both $1.5$B and $8$B. The result that matters most is negative and about the alternatives: a full model generation after GPT-4o, contradiction handling is still the weakest axis for GPT-5.2 and Claude~Opus~5, and both a verification-first few-shot prompt and an external symbolic guard \emph{degrade} it further. The prior has to be trained in, not requested at inference.

Two boundaries bound the claim. Transfer is largely absent: on four external datasets every $1.5$B checkpoint degenerates to a constant predictor at the majority-class baseline and the $8$B one inverts rather than resolves, with FOLIO at $8$B the lone positive---so what the pipeline installs is structural logical robustness, not general reasoning (\S\ref{sec:external}). And the Phase-1 oracle shares its forward-chaining engine with the benchmark labels, which is exactly why Phase~2 hands arbitration to an externally specified Lean~4 kernel (\S\ref{sec:phase2}). Next steps follow: end-to-end RLVF-Lean training through a persistent LeanDojo server, a closed-world axiom schema to lift the F-case gap, and math/proof benchmarks where the kernel is the natural verifier. Code, benchmark and verifier are released; see Appendix~\ref{app:limitations}.
