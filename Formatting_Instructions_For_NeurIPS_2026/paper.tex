...
(2) a two-phase generation path (verify, then deduce) that enforces verification-before-deduction, and
...
Guided by the structural failures identified above, we introduce the \textbf{Conflict-Aware Fusion (Fusion-Conflict)} framework. The core innovation is the imposition of a two-phase reasoning structure---\emph{verification then deduction}---within the Chain-of-Thought (CoT) generation path; this is a constraint on the \emph{generation trace}, not a modification of the underlying neural architecture or weights.
