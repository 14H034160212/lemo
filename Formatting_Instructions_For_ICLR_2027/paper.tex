...
premise is applied at $0.999$ and its contrapositive at $0.186$; three
configurations answer it correctly not once in $150$ attempts, and a wrong step
takes the step after it down in $77$--$100\%$ of cases. The same models complete
the rest of the chain at ceiling, so a total failure on one inference form
surfaces as an aggregate between $0.79$ and $0.93$---a degradation where the
measurement shows an absence. Raising the reasoning budget does not recover it. Reaching it required
...
contrapositive at $0.186$, with the upstream steps at ceiling; GPT-4.1 and
GPT-5.2 answer the contrapositive correctly not once in $150$ attempts, and the
downstream query fails alongside it in $77$--$100\%$ of cases
(\S\ref{sec:chain}).
...
reports modus tollens as harder than modus ponens; inside a chain it is absent
in three of eight configurations, reasoning effort does not recover it, and the
aggregates that conceal it read $0.79$--$0.93$.
...
\begin{table}[h]
\centering\small
\caption{Per-step accuracy on the matched-form chain, $150$ instances/arm.
Upstream is the mean of Q1 and Q2 over all three arms; MT\,$\to$\,Q4 is accuracy
on the step following the manipulated rule; \emph{prop.}\ is
$P(\text{Q4 wrong}\mid\text{Q3 wrong})$ on the MT arm. \emph{Effort} is the
\texttt{reasoning\_effort} parameter, which applies only to the GPT-5 family;
the GPT-4o and GPT-4.1 rows average two replicate runs of one configuration.
The aggregate column is the number a conventional evaluation would report.}
\label{tab:chain}
\begin{tabular}{llcccccc}
\toprule
\textbf{Model} & \textbf{Effort} & \textbf{Upstream} & \textbf{MP} & \textbf{MP$\neg$} & \textbf{MT} & \textbf{MT\,$\to$\,Q4} & \textbf{Aggregate} \\
\midrule
GPT-4o      & ---     & 0.962 & 0.967 & 1.000 & 0.087 & 0.297 & 0.841 \\
GPT-4.1     & ---     & 1.000 & 0.993 & 1.000 & \textbf{0.000} & 0.017 & 0.834 \\
GPT-5-nano  & default & 1.000 & 1.000 & 1.000 & 0.054 & 0.054 & 0.843 \\
GPT-5-nano  & medium  & 0.998 & 1.000 & 0.993 & 0.080 & 0.080 & 0.844 \\
GPT-5-mini  & default & 1.000 & 1.000 & 1.000 & 0.571 & 0.571 & 0.928 \\
GPT-5-mini  & medium  & 1.000 & 1.000 & 1.000 & 0.507 & 0.507 & 0.918 \\
GPT-5.2     & default & 0.582 & 0.747 & 0.500 & \textbf{0.000} & 0.000 & 0.499 \\
GPT-5.2     & medium  & 0.933 & 0.940 & 1.000 & \textbf{0.000} & 0.000 & 0.790 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{The failure is contraposition, not negation.} Across the seven
configurations whose upstream steps are intact, MP$\neg$ averages $0.999$ and
MT $0.186$; three answer MT correctly not once in $150$ attempts. Models that
apply \emph{``if not sleepy then young''} without a single error cannot apply
\emph{``if not young then not rough''} at all, on chains they otherwise
complete: negated antecedents are not the obstacle, running the implication
backwards is. GPT-5.2 at default effort is the one row where MP$\neg$ also
collapses ($0.500$), and there the upstream steps sit at $0.582$ and the
aggregate at $0.499$, indistinguishable from chance; we read it as a degenerate
configuration and exclude it from that average.

\paragraph{Reasoning budget does not buy it.} Raising
\texttt{reasoning\_effort} to medium moves MT by $+0.026$ on GPT-5-nano,
$-0.064$ on GPT-5-mini and $0.000$ on GPT-5.2, at $42$--$304$ reasoning tokens
per question. The one model that is not near-zero, GPT-5-mini at $0.571$, is
also the one that gets \emph{worse} with more effort. Whatever the deficit is,
it is not a budget the model can spend its way out of.

\paragraph{The cost is compound, and the aggregate hides it.} A wrong Q3 takes
Q4 with it in $77$--$100\%$ of cases, so one weak rule costs roughly two steps
...
aggregate between $0.79$ and $0.93$: GPT-4.1 scores $0.834$ overall while
answering zero of $150$ contrapositive queries correctly. This is the quantity
...
$0.186$---GPT-4.1 and GPT-5.2 not once in $150$ attempts---with the downstream
query failing alongside it, all of it invisible inside aggregates of
$0.79$--$0.93$. Where a perturbation fixes the label, accuracy alone cannot
