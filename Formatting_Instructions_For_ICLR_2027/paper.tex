...
\title{Present in Isolation and Absent in a Chain:\\Measuring Rule Loss in Multi-Step Reasoning}
...
Tell GPT-4.1 that Anne is rough, give it the rule \emph{``if someone is not
young then they are not rough''}, and ask whether Anne is young. It answers
correctly on $100$ of $100$ instances, and on $100$ matched instances whose
fact does not fire the rule it correctly declines to conclude anything. Now
derive \emph{``Anne is rough''} through two prior steps instead of stating it,
leave the rule, the wording and the question untouched, and the same model
answers correctly $0$ times in $600$ attempts---while answering the two steps
that precede it at $1.000$. The rule did not get harder and the model did not
stop having it. Two derived steps in front of it were enough.

We call this \textbf{rule loss}, and it is invisible to the way logical
reasoning is currently evaluated. It matters wherever a conclusion rests on a
chain rather than a single step---a policy engine composing clauses, an agent
carrying derived state forward---because there the steps preceding an inference
are the normal case, not a stress test.
...
\paragraph{Measuring any of this requires a control.} Perturbation benchmarks
assign labels deterministically, so a perturbed split is single-class and a
constant response saturates it---on our own contradiction split a model
answering \texttt{False} on sight scores $1.000$ while checking nothing. Every
claim below is therefore made on two-class splits against a stated majority
baseline (\S\ref{sec:v3control}), and under that control most of what this work
originally set out to claim did not survive.

\paragraph{Contributions.} The first is a measurement the existing benchmarks
are not built to make. The second is the set of controls it needs. The third is
a training-side result that is an instance of a mechanism already described,
and we mark it as such rather than claim it.

\textbf{(i)}~\emph{A rule is present in isolation and absent in a chain.}
Logical reasoning is evaluated by decomposing it into rules tested one at a
time---one rule per instance in \textsc{LogicBench}, atomic skills in
...
correctness. We hold a rule, its wording, the query and the oracle fixed and
vary only what precedes it (\S\ref{sec:chain}). GPT-4.1 applies the
contrapositive on $100$ of $100$ isolated instances, rejects $100$ of $100$
foils, and then applies the same rule correctly $0$ times in $600$ attempts
once two derived steps come first, with every other step of the chain at
ceiling. The literature reports modus tollens as harder than modus ponens,
measured one rule at a time, which is exactly the condition under which four of
our five models do not have the problem. Reasoning effort never recovers it,
and the aggregates that conceal it read $0.80$--$0.93$.
...
\textbf{(iii)}~\emph{Where a trained prior does and does not help}
(\S\ref{sec:v3control}). Conflict-aware SFT exceeds a constant responder on the
control; verifier-reward RL, from the same initialisation, collapses to one. That contrast is a specific case of a
...
every split of this benchmark. The capability it installs is propositional
premise validation and does not reach first-order reasoning
(\S\ref{sec:external}, \S\ref{sec:shortcut}).
...
\section{Benchmark and semantics}
\label{sec:method}
Every split used below is generated from one rule-based system, so an instance
and its perturbation differ in a single controlled way and nothing else. This
section fixes that generator and the semantics its labels carry; \S\ref{sec:chain}
uses it to build the matched arms, and \S\ref{sec:v3control} to build the
two-class control.
...
\section{Rule loss in multi-step reasoning}
\label{sec:chain}

\paragraph{Definition.} A model exhibits \textbf{rule loss} on a rule $r$ when
it applies $r$ correctly with $r$'s antecedent supplied as a fact, and fails to
apply the same $r$, in the same words, when that antecedent must instead be
derived. We measure it as $\mathrm{acc}_{0}(r) - \mathrm{acc}_{d}(r)$, the drop
between those two conditions on instances matched in every other
respect---predicates, wording, distractors, query, oracle---with $d$ the number
of steps derived before $r$ fires. It is a property of a
$(\text{model}, r, d)$ triple rather than of a model, and it is distinct from a
model simply lacking $r$, which $\mathrm{acc}_{0}(r)$ detects on its own.
...
\subsection{The failure is contraposition, not negation}
Across the seven
...
\subsection{The deficit belongs to the chain, not to the rule}
The reading so
...
\emph{Paired} scores the two halves of one instance jointly, as
\textsc{RULEBREAKERS} does, and is measured over all $200$ pairs; $^{\ast}$~marks
the three models whose halves are $1.000/1.000$ or $0.000/1.000$, which
determines the paired rate without a further run. $\Delta$ is taken from the
paired column. Chain figures are the MT column of Table~\ref{tab:chain}.}
\label{tab:isolated}
\begin{tabular}{lccccc}
\toprule
& \multicolumn{3}{c}{\textbf{MT in isolation}} & & \\
\cmidrule(lr){2-4}
\textbf{Model} & trigger & foil & paired & \textbf{MT in chain} & $\Delta$ \\
\midrule
GPT-4.1     & 1.000 & 1.000 & 1.000$^{\ast}$ & \textbf{0.000} & $\mathbf{-1.000}$ \\
GPT-5-mini  & 1.000 & 1.000 & 1.000$^{\ast}$ & 0.602 & $-0.398$ \\
GPT-4o      & 0.970 & 0.980 & 0.905 & 0.107 & $-0.798$ \\
GPT-5-nano  & 0.440 & 1.000 & 0.370 & 0.096 & $-0.274$ \\
GPT-5.2     & 0.000 & 1.000 & 0.000$^{\ast}$ & 0.000 & $\phantom{-}0.000$ \\
\bottomrule
\end{tabular}
\end{table}

Four of the five hold the rule, in whole or in part, and lose it. GPT-4.1 is
the clean case: it applies the contrapositive on $100$ of $100$ isolated
instances and rejects $100$ of $100$ foils, so the capability is present and is
not a surface cue---and the same rule, in the same words, is applied correctly
$0$ times in $600$ attempts once two derived steps precede it. GPT-4o keeps
both halves on $181$ of $200$ pairs and drops to $0.107$; GPT-5-nano holds the
rule only partially ($0.370$ paired, failing the trigger and never the foil,
the signature of not attempting the contraposition rather than botching it) and
still drops to $0.096$. GPT-5.2 is the instructive
...
\subsection{Reasoning budget does not buy it}
Raising
...
\subsection{The cost is compound, and the aggregate hides it}
A wrong Q3 takes
...
\section{Controls, and what a trained prior buys}
\label{sec:trained}

\S\ref{sec:chain} was measurable because every claim had a condition under
which the capability is absent---the foil. This section gives the control that
plays that role for the rest of the paper, and the training work this project
began as, which the control largely undoes. That work targets a different
failure, and we keep the two apart.
...
The failure targeted here is that a model given premises that contradict each
other keeps deducing from them instead of halting. We call it \textbf{Logic
Inertia}; it is not rule loss, since the rule is applied and it is the premise
set that has failed. The baseline pattern is uniform across BERT, Qwen2 and TinyLlama (Table~\ref{tab:all_models_deltas}, Appendix~\ref{app:baseline-table}): all three stay at $1.000$ on the base task and Variant~1, drop to $0.25$--$0.295$ under essential-rule removal (Variant~2), and collapse to $0.000$ under explicit contradictions (Variant~3).

Table~\ref{tab:comparison_results} summarises the trained objectives on
Qwen2-1.5B. Figures marked $\ddagger$ come from single-class splits and are not
evidence; they are kept only to show what \S\ref{sec:v3control} changes.
...
The sharpest challenge to a training-time fix is that the behaviour might simply be promptable. We test two frontier models under three inference-time regimes on the same stratified $200$-row-per-split protocol ($800$ questions/split): zero-shot; a \emph{verification-first few-shot} prompt instructing the model to normalise rules, check consistency and halt before deducing; and a \emph{neuro-symbolic guard}, a naive LLM whose answers are overridden to \texttt{False} wherever an external checker finds a contradiction in the raw facts, in the spirit of \citet{vericot2026} (Table~\ref{tab:frontier_prompt}).
...
\paragraph{What the failure looks like.} Given $\{A \to B,\; A,\; \neg B\}$, GPT-4o often surfaces the inconsistency in its trace and proceeds anyway, overriding the $\neg B$ it just acknowledged (Appendix~\ref{app:qualitative}). We read this as detection and halting being separate capabilities, but the reading is qualitative: \S\ref{sec:shortcut} shows trace mentions are not a detection signal.
...
An earlier draft also read a sensitivity profile off the Variant-1 and
Variant-4 splits. Those are single-class too, so we withdraw that reading on
the same grounds as the rest; the per-variant figures stay in
Appendix~\ref{app:ablation}. What we can report under a control is external
transfer, and it is negative.
...
\section{Conclusion}
A rule a model applies at ceiling on its own is applied zero times once two
derived steps precede it. Across five frontier models a negated premise is
handled at $0.999$ and its contrapositive at $0.209$, the downstream query
fails alongside it, and stripped of the chain four of the five recover the rule
at or near ceiling---so what the chain costs is not a capability they lacked.
Reasoning budget does not buy it back, and aggregates of $0.80$--$0.93$ conceal
all of it. Rule loss is a property of a model, a rule and a depth, and the
isolated condition is the one under which the existing literature measures.

We also set out to show that a verify-before-deduce prior repairs contradiction
handling, and built controls strong enough to test it. Most of that did not
survive: of four trained objectives only conflict-aware SFT exceeds a constant
responder, at $0.698 \pm 0.054$ against a $0.5835$ baseline, weaker at 8B rather
than stronger and resting partly on a surface cue that co-occurs with
contradiction in training with probability $1.000$; a symbolic solver scores
$1.000$ on every split here. The lesson is the measurement's: where a
perturbation fixes the label, accuracy alone cannot tell a capability from a
constant.
