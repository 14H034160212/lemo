...
COLORS = ["green", "blue", "red", "yellow", "purple", "orange", "silver",
          "golden", "crimson", "violet"]
# v1 hardcoded the chain to cold/rough/young/nice, so 3000 rows held only 329
# distinct structures -- a 9x replication that every question-level interval on
# that split had to be widened for, and few enough to memorise. Drawing the
# chain from a pool makes each instance structurally distinct. The four
# positions keep their roles: attrs[0] follows from the colour, attrs[1] from
# it, attrs[2] is reachable only through the contrapositive rule, and attrs[3]
# depends on attrs[2].
ATTR_POOL = ["cold", "rough", "young", "nice", "quick", "bright", "calm",
             "eager", "firm", "gentle", "humble", "keen", "lively", "mild",
             "noble", "patient", "quiet", "ready", "steady", "tidy"]
...
def questions_for(name, attrs):
    return [f"Q{i+1}: {name} is {a}." for i, a in enumerate(attrs)]


def base_chain(name, drop=None, attrs=None):
    ...
    rules = []
    if drop != "entry":
        rules += [rule(c1, a0), rule(c2, a0)]
    rules.append(rule(a0, a1))
    if drop != "young":
        rules.append(rule(f"not {a2}", f"not {a1}"))
    rules += [rule(a2, a0), rule(a2, a3),
              "If someone is tall then they are warm."]
    random.shuffle(rules)
    return facts, rules, (c1, c2)


def oracle_answers(facts, rules, name, attrs):
    ...
    out = []
    for q in questions_for(name, attrs):
...
def build(num_groups, seed=0):
    ...
    for _ in range(num_groups):
        ...
        gid = str(uuid.uuid4())
        attrs = random.sample(ATTR_POOL, 4)   # per-instance chain

        # ---- 1. contradiction: negate something the chain derives ----
        facts, rules, (c1, c2) = base_chain(name, attrs=attrs)
        extra = f"{name} is not {random.choice(attrs)}"
        f = facts + [extra]
        ans, contra = oracle_answers(f, rules, name, attrs)
        if not contra:
            stats["rejected"] += 1            # oracle disagreed -> drop it
        else:
            rows.append((gid, "contra", f, rules, name, ans, attrs))
            stats["contra"] += 1
        ...
        if contra:
            ...
            rows.append((gid, "consistent_disj", f, rules, name, ans, attrs))
        ...
        facts, rules, (c1, c2) = base_chain(
            name, drop=random.choice(["young", "entry"]), attrs=attrs)
        base_ans, _ = oracle_answers(facts, rules, name, attrs)
        ...
        if not underivable:
            ...
            if contra:
                ...
                rows.append((gid, "consistent_mixed", f, rules, name, ans, attrs))
...
def main():
    ...
    out = "data/test_variant3_mixed_v2.csv"
    ...
    with open(out, "w", newline="", encoding="utf-8") as fh:
        ...
        w.writeheader()
        for gid, typ, facts, rules, name, ans, attrs in rows:
            w.writerow({
                "group_id": gid, "type": typ,
                "facts": " | ".join(facts), "rules": " | ".join(rules),
                "questions": " | ".join(questions_for(name, attrs)),
                "answers": " | ".join(ans), "equiv_laws_used": "",
            })

    allans = [a for *_, ans, _ in rows for a in ans]
    t = allans.count("T")
